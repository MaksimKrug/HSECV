...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, pretrained=True, weight_path=""):
        super().__init__()
        second_kernel = 3
        max_pool = True
        grmul = 1.7
        ...
        self.base.append(
            ConvLayer(in_channels=3, out_channels=32, kernel=3, stride=2, bias=False)
        )

        # Second Layer
        self.base.append(ConvLayer(32, 64, kernel=second_kernel))

        # Maxpooling or DWConv3x3 downsampling
        if max_pool:
            self.base.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        else:
            self.base.append(DWConvLayer(64, 64, stride=2))
...
def hardnet(pretrained=True):
    """
    Get HardNet backbone
    """
    model = HarDNet()
    if pretrained:
